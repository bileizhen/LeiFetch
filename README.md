...
- 四种模式：不使用代理、系统代理（默认）、手动配置、自动（推荐）
- 默认「系统代理」，与引入代理前的行为一致，不改变既有用户的线路；需要更激进时才切到「自动」
- 手动配置支持 HTTP 与 SOCKS5、账号密码认证，以及直连名单（域名后缀、`*`、`<local>`）
- 自动模式优先跟随系统代理，其次探测本机常见代理端口，都不可用时直连；候选端口覆盖 Clash、Clash Verge、v2rayN、sing-box、Privoxy 与抓包工具的默认监听
- 探测先看端口是否在监听，再按 HTTP CONNECT / SOCKS5 握手确认确实是代理，结果缓存 5 分钟；只做本机 TCP 连接与协议握手，不产生下载流量
- 下载、GitHub 镜像测速与 Firefox 插件探测共用同一份配置，代理页可一键测试当前线路并报告实际走的线路与首字节延迟
...
- 设置 → 诊断 → 日志：实时查看运行日志，新记录自动贴底跟随，往回翻会暂停跟随并可一键回到最新
- 覆盖下载、NSFX 内核、GitHub 镜像、代理与捕获路径；插件进程（Firefox、系统下载器）的日志经捕获提供者回传，与应用进程写在同一条时间线上
- 按级别（信息 / 警告 / 错误）、来源与关键字筛选，来源筛选项按实际出现过的来源生成
- 一键导出诊断包：`logs.txt`（本页日志）、`summary.json`（版本与设置摘要）、`logcat.txt`（本进程系统日志片段），可保存到任意位置或直接分享
- 日志只保留最近 600 条且仅存于内存，不落盘；地址只记录主机名，不含查询串
...
日志要能贴出来求助，就不能带着可能含签名或凭据的完整链接。因此只记录主机名（例如 `github.com`），查询串一律不写入，导出的诊断包同样只有主机名。
...
自动模式只探测本机回环地址上的常见代理端口，代理客户端需要监听 `127.0.0.1`。端口不在候选列表里，或代理在局域网内其它设备上时，请在「设置 → 网络 → 代理」选择「手动配置」并填写地址与端口。若代理客户端以 VPN / TUN 方式接管流量，系统本身已透明转发，保持「自动」或「不使用代理」都可正常工作。

### 代理设置对哪些请求生效？

下载（含断点续传的每一段）、GitHub 镜像测速与 Firefox 插件的独立探测都走同一份配置。直连名单内的地址始终直连。代理只作用于 LeiFetch 自己发起的请求，不会改变被接管应用自身的网络行为。

## 隐私

- 下载探测不消费完整文件正文；日志记录回退原因，但不记录可能含凭据的完整下载 URL，地址只保留主机名。
- 关于页的开发组与贡献者名单会从腾讯 QQ 头像 CDN 加载对应头像；不发生其它请求。
- 启用 GitHub 镜像加速时，对应任务的 GitHub 地址会经所选镜像站转发；镜像测速只请求 Range 首字节，不消费文件正文。
- 启用代理后，下载、镜像测速与 Firefox 探测的请求都经所配置的代理转发；自动探测只连接本机回环端口并与代理握手，不发起外部请求，也不读取其它应用的代理配置内容。
